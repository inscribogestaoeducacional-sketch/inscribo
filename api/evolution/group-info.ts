...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  const { instanceName, groupJid } = req.query
  if (!instanceName || !groupJid) return res.status(400).json({ error: 'Missing params' })
  try {
    const response = await fetch(`${EVOLUTION_URL}/group/findGroupInfos/${instanceName}?groupJid=${groupJid}`, {
