...
export default function CampaignGeneratorModal({
  ...
  useEffect(() => {
    if (!isOpen) { setStep(1); setError(null); setGeneratedPlan(null); setAdjustedPlan(null); setAiAnalysis(null); setGenProgress(0); setGenMsgIdx(0) }
  }, [isOpen])
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (newHistData.length) {
      setHistoricalData(prev => {
        const all = [...prev,...newHistData].reduce<HistoricalYear[]>((acc,r) => { if(!acc.find(x=>x.year===r.year)) acc.push(r); return acc },[]).sort((a,b)=>a.year-b.year)
        return all
      })
  ...
  const analyzeWithAI = useCallback(async () => {
    ...
    try {
      ...
      const hd = historicalData.filter(d=>d.total_students>0).sort((a,b)=>a.year-b.year)
      ...
      const novatosTrend = validFiles.length>=2
        ? validFiles[validFiles.length-1].novatos > validFiles[0].novatos ? 'up'
        : validFiles[validFiles.length-1].novatos < validFiles[0].novatos ? 'down' : 'stable'
        : 'stable'
      const retTrend = retRates.length>=2
        ? retRates[retRates.length-1]>retRates[0] ? 'up' : retRates[retRates.length-1]<retRates[0] ? 'down' : 'stable'
        : 'stable'
      ...
      const analysis: AIAnalysis = {
        ...
        key_insight: novatosTrend==='down'
          ? `Novatos caíram ${novPctChange!==null?Math.abs(novPctChange)+'%':''}  nos últimos anos — sem ação, a escola perde base continuamente`
          : `Retenção de ${(avgRet*100).toFixed(1)}% é ${avgRet>=0.85?'excelente — foco total em captação de novatos':avgRet>=0.75?'boa — há espaço para melhorar a fidelização':'abaixo do ideal — programa de retenção é urgente'}`,
        risk: avgRet<0.80
          ? 'Retenção abaixo de 80% — escola perde mais alunos do que deveria a cada ano'
          : novatosTrend==='down'
          ? 'Queda consistente em novatos pode comprometer o volume total nos próximos anos'
          : 'Manter investimento em captação para compensar saídas naturais de formandos'
      }
...
function Step1Upload({ erpFiles, setErpFiles, loadingFile, multiFileProgress, fileInputRef, onFileChange, historicalData, setHistoricalData }: { erpFiles: ErpFileEntry[]; setErpFiles: React.Dispatch<React.SetStateAction<ErpFileEntry[]>>; loadingFile: boolean; multiFileProgress: string|null; fileInputRef: React.RefObject<HTMLInputElement>; onFileChange: (e: React.ChangeEvent<HTMLInputElement>) => void; historicalData: HistoricalYear[]; setHistoricalData: React.Dispatch<React.SetStateAction<HistoricalYear[]>> }) {
  ...
  return (
    <div style={{paddingBottom:24}}>
      ...
      {erpFiles.length>0&&<div style={{display:'flex',flexDirection:'column',gap:8,marginBottom:16}}>
        {erpFiles.map((f,i)=>(
          <div key={i} style={{display:'flex',alignItems:'center',gap:12,padding:'12px 16px',borderRadius:12,background:f.error?'#fef2f2':'#f0fdf4',border:`1px solid ${f.error?'#fca5a5':'#bbf7d0'}`}}>
            <div style={{width:36,height:36,borderRadius:10,background:f.error?'#fee2e2':'#d1fae5',display:'flex',alignItems:'center',justifyContent:'center',flexShrink:0}}>{f.error?<AlertTriangle size={16} color="#dc2626"/>:<FileText size={16} color="#16a34a"/>}</div>
            <div style={{flex:1,minWidth:0}}>
              <div style={{fontSize:13,fontWeight:600,color:f.error?'#991b1b':'#166534',overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{f.name}</div>
              {!f.error&&f.year>0&&<div style={{fontSize:11,color:'#6b7280',marginTop:2}}>{f.year} · {f.total.toLocaleString('pt-BR')} alunos · {f.novatos} novatos · {f.veterans} veteranos · {f.total>0?((f.veterans/f.total)*100).toFixed(1):'?'}% retenção</div>}
              {f.error&&<div style={{fontSize:11,color:'#991b1b',marginTop:2}}>Não foi possível ler — verifique o formato</div>}
            </div>
      ...
      {erpFiles.length===0&&(
        <div style={{background:'#F8FAFC',borderRadius:12,padding:16,border:'1px solid #E2E8F0'}}>
          ...
          <div style={{display:'grid',gridTemplateColumns:'70px 1fr 1fr 1fr 1fr',gap:8,marginBottom:6}}>
            {['Ano','Total','Novatos','Remat.','Transfer.'].map(h=><span key={h} style={{fontSize:10,color:'#94A3B8',fontWeight:600,textAlign:'center'}}>{h}</span>)}
          </div>
          {historicalData.map((row,i)=>(
            <div key={i} style={{display:'grid',gridTemplateColumns:'70px 1fr 1fr 1fr 1fr',gap:8,marginBottom:8}}>
          ...
          {historicalData.length<5&&<button onClick={()=>setHistoricalData(d=>[...d,{year:new Date().getFullYear()-1-d.length,total_students:0,new_enrollments:0,reenrollments:0,transfers:0}])} style={{fontSize:12,color:'#00A896',background:'none',border:'none',cursor:'pointer',padding:'4px 0'}}>+ Adicionar ano</button>}
        </div>
      )}
...
function Step2Analysis({ erpFiles, aiAnalysis, loading, error, onRetry }: { erpFiles: ErpFileEntry[]; aiAnalysis: AIAnalysis|null; loading: boolean; error: string|null; onRetry: ()=>void }) {
  const validFiles = erpFiles.filter(f=>!f.error&&f.year>0).sort((a,b)=>a.year-b.year)
  const chartData = validFiles.map(f=>({ year:String(f.year), novatos:f.novatos, veteranos:f.veterans, total:f.total, retencao:f.total>0?+((f.veterans/f.total)*100).toFixed(1):0 }))

  return (
    <div style={{paddingBottom:24}}>
      <h2 style={{fontSize:22,fontWeight:800,color:'#1A2B4A',marginBottom:6}}>Diagnóstico da sua escola</h2>
      <p style={{fontSize:14,color:'#64748B',marginBottom:20}}>A IA analisou {validFiles.length} ano{validFiles.length>1?'s':''} de histórico.</p>
        ...
        <div style={{display:'flex',flexDirection:'column',gap:16}}>
          ...
          <div style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:12}}>
            {[
              ...
              <div key={i} style={{background:card.bg,borderRadius:12,padding:'14px 16px',border:'1px solid #E2E8F0'}}>
                <p style={{margin:'0 0 8px',fontSize:10,fontWeight:700,color:'#94A3B8',textTransform:'uppercase',letterSpacing:'0.05em'}}>{card.label}</p>
                <p style={{margin:'0 0 4px',fontSize:22,fontWeight:800,color:card.color,lineHeight:1}}>{card.value}</p>
                <p style={{margin:0,fontSize:11,color:'#64748B'}}>{card.sub}</p>
              </div>
            ))}
          </div>
          {/* Análise IA */}
          <div style={{background:'linear-gradient(135deg,#1A2B4A,#2d4494)',borderRadius:14,padding:20}}>
            <div style={{display:'flex',alignItems:'center',gap:8,marginBottom:12}}><Sparkles size={16} color="#fff"/><span style={{fontSize:13,fontWeight:700,color:'#fff'}}>Análise da IA</span></div>
            <p style={{margin:'0 0 14px',fontSize:13,color:'rgba(255,255,255,0.9)',lineHeight:1.7}}>{aiAnalysis.summary}</p>
            <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:10}}>
              <div style={{background:'rgba(0,168,150,0.2)',borderRadius:10,padding:'10px 14px'}}><p style={{margin:'0 0 4px',fontSize:10,fontWeight:700,color:'#5eead4',textTransform:'uppercase'}}>💡 Insight principal</p><p style={{margin:0,fontSize:12,color:'#ccfbf1',lineHeight:1.5}}>{aiAnalysis.key_insight}</p></div>
              <div style={{background:'rgba(239,68,68,0.15)',borderRadius:10,padding:'10px 14px'}}><p style={{margin:'0 0 4px',fontSize:10,fontWeight:700,color:'#fca5a5',textTransform:'uppercase'}}>⚠️ Risco identificado</p><p style={{margin:0,fontSize:12,color:'#fee2e2',lineHeight:1.5}}>{aiAnalysis.risk}</p></div>
            </div>
          </div>
          {/* Gráfico */}
          {chartData.length>=2&&(
            <div style={{background:'#fff',borderRadius:14,border:'1px solid #E2E8F0',padding:20}}>
              ...
              <ResponsiveContainer width="100%" height={200}>
                <ComposedChart data={chartData}>
                  ...
                  <Tooltip contentStyle={{borderRadius:10,fontSize:12}} formatter={(val,name)=>[name==='retencao'?`${val}%`:val,name==='novatos'?'Novatos':name==='veteranos'?'Veteranos':'Retenção']}/>
          ...
          <div style={{background:'#F0FDF4',borderRadius:12,padding:'14px 16px',border:'1px solid #BBF7D0',display:'flex',alignItems:'center',gap:14}}>
            ...
            <div>
              <p style={{margin:0,fontSize:13,fontWeight:700,color:'#166534'}}>Período sugerido pela IA</p>
              <p style={{margin:'2px 0 0',fontSize:12,color:'#16a34a'}}>{new Date(aiAnalysis.suggested_start_date+'T12:00:00').toLocaleDateString('pt-BR',{month:'long',year:'numeric'})} → {new Date(aiAnalysis.suggested_end_date+'T12:00:00').toLocaleDateString('pt-BR',{month:'long',year:'numeric'})}</p>
            </div>
            <div style={{marginLeft:'auto',textAlign:'right'}}>
              <p style={{margin:0,fontSize:11,color:'#94A3B8'}}>Novatos sugeridos</p>
              <p style={{margin:'2px 0 0',fontSize:20,fontWeight:800,color:'#00A896'}}>{aiAnalysis.suggested_new_students}</p>
            </div>
          </div>
        </div>
      )}
...
function Step4Generating({ loading, progress, msgIdx, msgs, error, onRetry }: { loading:boolean; progress:number; msgIdx:number; msgs:string[]; error:string|null; onRetry:()=>void }) {
  return (
    <div style={{paddingBottom:24,paddingTop:16}}>
      ...
      <p style={{fontSize:14,color:'#64748B',marginBottom:32}}>A IA está montando as metas com base no histórico e sazonalidade real da escola.</p>
...
// ─── Passo 5 — placeholder (implementado na Parte 2) ─────────────
function Step5Review(_props: { plan: GeneratedPlan; basePlan: GeneratedPlan; ambitiousLevel: number; campaignYear: number; startDate: string; endDate: string; erpFiles: ErpFileEntry[]; totalExits: number; currentStudents: number; monthsUntilCampaign: number; campaignStartMonth: string; onAmbitiousChange: (l:number)=>void; onManualTargets: (n:number,r:number)=>void; onUpdateCell: (i:number,f:keyof MonthlyTarget,v:number)=>void; onRegenerate?: ()=>void }) {
  return <div style={{padding:'40px 0',textAlign:'center',color:'#94A3B8'}}>Carregando revisão...</div>
}// ─────────────────────────────────────────────────────────────────
// PARTE 2: Step5Review — substitui o placeholder da Parte 1
// Cole este conteúdo no final do arquivo, substituindo o Step5Review
// ─────────────────────────────────────────────────────────────────
import React, { useState, useEffect } from 'react'
import {
  Sparkles, RefreshCw, SlidersHorizontal, Edit3,
  Users, DollarSign, TrendingUp, Check, AlertTriangle
} from 'lucide-react'
import { getCampaignMonths, scalePlan, redistributePlan, S } from './CampaignGeneratorModal'
import type { GeneratedPlan, MonthlyTarget, AIAnalysis } from './CampaignGeneratorModal'
...
export function Step5Review({ plan, basePlan, ambitiousLevel, campaignYear, startDate, endDate, erpFiles, totalExits, currentStudents, monthsUntilCampaign, campaignStartMonth, onAmbitiousChange, onManualTargets, onUpdateCell, onRegenerate }: {
  ...
  const totals = plan.monthly_targets.reduce((acc,m) => ({
    ...
    investment: acc.investment + m.investment_suggested,
    new: acc.new + (m.enrollments_new??0),
    returning: acc.returning + (m.enrollments_returning??0)
  }), { registrations:0,schedules:0,visits:0,enrollments:0,investment:0,new:0,returning:0 })

  // Rematrícula acumulada
  let accReen = 0
  const reenRows = plan.monthly_targets.map((m,i) => {
    const mReen = m.enrollments_returning ?? 0
    accReen += mReen
    return { label: campaignMonths[i]?.label ?? `${m.month}/${m.year}`, monthNum: campaignMonths[i]?.month ?? 0, mReen, accReen, pct: reenrollTarget>0 ? (accReen/reenrollTarget)*100 : 0 }
  })
  ...
  // Meses críticos (out/nov)
  const criticalTotal = reenRows.filter(r=>[10,11].includes(r.monthNum)).reduce((s,r)=>s+r.mReen,0)
      ...
      <div style={{ marginBottom:20 }}>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'1fr 220px', gap:12, marginBottom: showManual ? 12 : 0 }}>
          ...
          <div style={{ background:'#F8FAFC', borderRadius:12, padding:'12px 14px', border:'1px solid #E2E8F0' }}>
            ...
            <p style={{ margin:'6px 0 0', fontSize:9, color:'#94A3B8', textAlign:'center' }}>Escala proporcionalmente sem chamar a IA</p>
          </div>
        </div>

        {/* Painel ajuste manual */}
        {showManual && (
          <div style={{ background:'#F0FDF4', borderRadius:12, padding:16, border:'1px solid #BBF7D0' }}>
            ...
            <div style={{ display:'flex', gap:8, marginBottom:12 }}>
              {basePlan && [
                ...
                <button key={s.label} onClick={()=>{ setManualNew(s.n); setManualReen(s.r); onManualTargets(s.n,s.r) }} style={{ flex:1, padding:'8px 6px', borderRadius:8, border:'1px solid #BBF7D0', background:'#fff', cursor:'pointer', fontSize:11, fontWeight:600, color:'#065F46' }}>
                  {s.label}<br/><span style={{ fontSize:10, color:'#94A3B8', fontWeight:400 }}>{s.n} novos · {s.r} remat.</span>
                </button>
              ))}
            </div>
            ...
            <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr auto', gap:10, alignItems:'end' }}>
              <div>
                <label style={{ ...S.label, color:'#065F46' }}>Novatos (total)</label>
                <input type="number" min={0} value={manualNew} onChange={e=>setManualNew(parseInt(e.target.value)||0)} style={{ ...S.input, border:'1.5px solid #00A896', fontWeight:700, color:'#00A896' }}/>
              </div>
              <div>
                <label style={{ ...S.label, color:'#065F46' }}>Rematrículas (total)</label>
                <input type="number" min={0} value={manualReen} onChange={e=>setManualReen(parseInt(e.target.value)||0)} style={{ ...S.input, border:'1.5px solid #3B82F6', fontWeight:700, color:'#3B82F6' }}/>
              </div>
              <button onClick={()=>onManualTargets(manualNew,manualReen)} style={{ height:40, padding:'0 18px', borderRadius:9, background:'#00A896', color:'#fff', border:'none', fontSize:12, fontWeight:700, cursor:'pointer', display:'flex', alignItems:'center', gap:6, whiteSpace:'nowrap' }}>
                <RefreshCw size={12}/> Redistribuir
              </button>
            </div>
          </div>
        )}
          ...
          <table style={{ width:'100%', borderCollapse:'collapse', fontSize:12 }}>
            ...
            <tbody>
              {plan.monthly_targets.map((m,i) => {
                ...
                const ne = m.enrollments_new ?? 0
                const re = m.enrollments_returning ?? 0
                const te = ne + re || m.enrollments || 0
                const cpa = te>0 ? Math.round((m.investment_suggested||0)/te) : 0
                const isKeyMonth = [10,11].includes(campaignMonths[i]?.month ?? 0)
                return (
                  <tr key={i} style={{ borderBottom:'1px solid #F8FAFC', background: isKeyMonth ? '#FFFBEB' : i%2===0?'#fff':'#FAFAFA' }}>
                    <td style={{ padding:'6px 12px', fontWeight:600, color:'#1A2B4A', whiteSpace:'nowrap', fontSize:11 }}>
                      {label}{isKeyMonth && <span style={{ marginLeft:4, fontSize:9, color:'#f59e0b', fontWeight:700 }}>★</span>}
                    </td>
                    {/* Captação editável */}
                    {(['registrations','schedules','visits'] as const).map(field => (
                      <td key={field} style={{ padding:'3px 4px', textAlign:'center', background:'#F8FAFF' }}>
                        <input type="number" style={{ width:54, padding:'3px 4px', borderRadius:5, border:'1px solid #BFDBFE', textAlign:'center', fontSize:11, outline:'none', background:'#fff' }} value={m[field]||0} onChange={e=>onUpdateCell(i,field,parseFloat(e.target.value)||0)}/>
                      </td>
                    ))}
                    {/* Novatos */}
                    <td style={{ padding:'3px 4px', textAlign:'center', background:'#F0FDFB' }}>
                      <input type="number" style={{ width:54, padding:'3px 4px', borderRadius:5, border:'1px solid #99F6E4', textAlign:'center', fontSize:11, outline:'none', background:'#fff', fontWeight:600, color:'#0d9488' }} value={ne} onChange={e=>onUpdateCell(i,'enrollments_new',parseFloat(e.target.value)||0)}/>
                    </td>
                    {/* Rematrícula */}
                    <td style={{ padding:'3px 4px', textAlign:'center', background:'#F0FDFB' }}>
                      <input type="number" style={{ width:54, padding:'3px 4px', borderRadius:5, border:'1px solid #BBF7D0', textAlign:'center', fontSize:11, outline:'none', background:'#fff', fontWeight:600, color:'#16a34a' }} value={re} onChange={e=>onUpdateCell(i,'enrollments_returning',parseFloat(e.target.value)||0)}/>
                    </td>
                    {/* Total */}
                    <td style={{ padding:'6px 8px', textAlign:'center', fontWeight:800, color:'#065F46', fontSize:12, background:'#F0FDFB' }}>{te}</td>
                    {/* Financeiro */}
                    <td style={{ padding:'3px 4px', textAlign:'center', background:'#FAF5FF' }}>
                      <input type="number" style={{ width:64, padding:'3px 4px', borderRadius:5, border:'1px solid #DDD6FE', textAlign:'center', fontSize:11, outline:'none', background:'#fff' }} value={m.investment_suggested||0} onChange={e=>onUpdateCell(i,'investment_suggested',parseFloat(e.target.value)||0)}/>
                    </td>
              ...
              <tr style={{ background:'#F0FDFB', borderTop:'2px solid #00A896' }}>
                ...
                <td style={{ padding:'9px 8px', textAlign:'center', fontWeight:700, fontSize:11, color:'#1D4ED8' }}>{totals.registrations}</td>
                <td style={{ padding:'9px 8px', textAlign:'center', fontWeight:700, fontSize:11, color:'#1D4ED8' }}>{totals.schedules}</td>
                <td style={{ padding:'9px 8px', textAlign:'center', fontWeight:700, fontSize:11, color:'#1D4ED8' }}>{totals.visits}</td>
                <td style={{ padding:'9px 8px', textAlign:'center', fontWeight:800, fontSize:12, color:'#0d9488' }}>{totals.new}</td>
                <td style={{ padding:'9px 8px', textAlign:'center', fontWeight:800, fontSize:12, color:'#16a34a' }}>{totals.returning}</td>
                <td style={{ padding:'9px 8px', textAlign:'center', fontWeight:800, fontSize:13, color:'#065F46' }}>{totals.enrollments}</td>
                <td style={{ padding:'9px 8px', textAlign:'center', fontWeight:700, fontSize:11, color:'#7C3AED' }}>R$ {totals.investment.toLocaleString('pt-BR')}</td>
                <td style={{ padding:'9px 8px', textAlign:'center', fontWeight:700, fontSize:11, color:'#7C3AED' }}>{totals.enrollments>0?`R$ ${Math.round(totals.investment/totals.enrollments).toLocaleString('pt-BR')}`:'—'}</td>
              </tr>
            </tbody>
          </table>
        </div>
        <p style={{ margin:'8px 0 0', fontSize:11, color:'#94A3B8' }}>★ Meses críticos de rematrícula · Azul = captação · Verde = matrículas · Roxo = financeiro · Células editáveis</p>
      </div>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr 1fr', gap:12 }}>
        ...
        <div style={{ background:'#fff', borderRadius:14, border:'1px solid #E2E8F0', padding:16 }}>
          <div style={{ display:'flex', alignItems:'center', gap:8, marginBottom:12 }}>
            ...
            <span style={{ fontSize:12, fontWeight:700, color:'#1A2B4A' }}>Rematrícula acumulada</span>
          </div>
          <div style={{ display:'flex', flexDirection:'column', gap:6 }}>
            {reenRows.filter(r=>r.mReen>0).map((r,i) => {
              const pctColor = r.pct>=80?'#16a34a':r.pct>=40?'#f59e0b':'#3b82f6'
              return (
                <div key={i}>
                  <div style={{ display:'flex', justifyContent:'space-between', marginBottom:3 }}>
                    <span style={{ fontSize:11, color:'#475569', fontWeight:600 }}>{r.label}</span>
                    <span style={{ fontSize:11, fontWeight:700, color:pctColor }}>{r.pct.toFixed(0)}% · {r.accReen}</span>
                  </div>
                  <div style={{ height:5, background:'#E2E8F0', borderRadius:999, overflow:'hidden' }}>
                    <div style={{ height:'100%', width:`${Math.min(100,r.pct)}%`, background:pctColor, borderRadius:999, transition:'width 0.4s' }}/>
                  </div>
                </div>
              )
            })}
            {reenRows.every(r=>r.mReen===0) && <p style={{ fontSize:12, color:'#94A3B8', textAlign:'center', margin:'8px 0' }}>Sem metas de rematrícula definidas</p>}
          </div>
          <div style={{ marginTop:12, paddingTop:10, borderTop:'1px solid #E2E8F0', display:'flex', justifyContent:'space-between', fontSize:11 }}>
            <span style={{ color:'#64748B' }}>Meta total:</span>
            <span style={{ fontWeight:700, color:'#3B82F6' }}>{reenrollTarget} alunos</span>
          </div>
        </div>
        ...
        <div style={{ background:'#fff', borderRadius:14, border:'1px solid #E2E8F0', padding:16 }}>
          <div style={{ display:'flex', alignItems:'center', gap:8, marginBottom:12 }}>
            ...
            <span style={{ fontSize:12, fontWeight:700, color:'#1A2B4A' }}>Riscos identificados</span>
          </div>
            ...
            ? plan.key_risks.map((r,i) => (
                <div key={i} style={{ display:'flex', gap:8, marginBottom:10, alignItems:'flex-start' }}>
                  <div style={{ width:18, height:18, borderRadius:'50%', background:'#FEF3C7', display:'flex', alignItems:'center', justifyContent:'center', flexShrink:0, marginTop:1 }}>
                    <span style={{ fontSize:9, fontWeight:800, color:'#D97706' }}>{i+1}</span>
                  </div>
                  <span style={{ fontSize:11, color:'#78350F', lineHeight:1.55 }}>{r}</span>
                </div>
              ))
            : <p style={{ fontSize:12, color:'#94A3B8', textAlign:'center', margin:'16px 0' }}>Nenhum risco crítico identificado</p>}
          {criticalTotal>0 && (
            <div style={{ marginTop:8, paddingTop:10, borderTop:'1px solid #E2E8F0', background:'#FFFBEB', borderRadius:8, padding:10 }}>
              <p style={{ margin:0, fontSize:11, color:'#92400E', fontWeight:600 }}>★ Out/Nov são críticos</p>
              <p style={{ margin:'2px 0 0', fontSize:11, color:'#78350F' }}>{criticalTotal} rematrículas esperadas ({criticalPct}% do total)</p>
            </div>
          )}
        </div>
        ...
        <div style={{ background:'#fff', borderRadius:14, border:'1px solid #E2E8F0', padding:16 }}>
          <div style={{ display:'flex', alignItems:'center', gap:8, marginBottom:12 }}>
            <Check size={14} color="#00A896"/>
            <span style={{ fontSize:12, fontWeight:700, color:'#1A2B4A' }}>Ações prioritárias</span>
          </div>
          {plan.key_actions?.length>0
            ? plan.key_actions.map((a,i) => (
                <div key={i} style={{ display:'flex', gap:8, marginBottom:10, alignItems:'flex-start' }}>
                  ...
                  <span style={{ fontSize:11, color:'#047857', lineHeight:1.55 }}>{a}</span>
                </div>
              ))
            : <p style={{ fontSize:12, color:'#94A3B8', textAlign:'center', margin:'16px 0' }}>Nenhuma ação registrada</p>}
          {plan.pre_campaign && monthsUntilCampaign>0 && (
            <div style={{ marginTop:8, paddingTop:10, borderTop:'1px solid #E2E8F0', background:'#FFFBEB', borderRadius:8, padding:10 }}>
              <p style={{ margin:0, fontSize:11, color:'#92400E', fontWeight:600 }}>Antes de {campaignStartMonth}</p>
              {plan.pre_campaign.key_actions.slice(0,2).map((a,i)=><p key={i} style={{ margin:'4px 0 0', fontSize:10, color:'#78350F' }}>• {a}</p>)}
            </div>
          )}
