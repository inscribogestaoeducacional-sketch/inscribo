...
interface PreCampaign {
  period: string; months_count: number; focus_areas: string[]; key_actions: string[]
...
function getCampaignMonths(startDate: string, endDate: string) {
  ...
  while (current <= end) {
    months.push({ label: `${monthNames[current.getMonth()]}/${current.getFullYear()}`, month: current.getMonth() + 1, year: current.getFullYear(), period: `${current.getFullYear()}-${String(current.getMonth() + 1).padStart(2, '0')}` })
...
function scalePlan(plan: GeneratedPlan, factor: number): GeneratedPlan {
  ...
  scaled.monthly_targets = scaled.monthly_targets.map(m => {
    const newEnr = Math.round((m.enrollments_new ?? 0) * factor)
    const retEnr = Math.round((m.enrollments_returning ?? 0) * factor)
    const totalEnr = newEnr + retEnr
    const reg = Math.round((m.registrations || 0) * factor)
    const sch = Math.round((m.schedules || 0) * factor)
    const vis = Math.round((m.visits || 0) * factor)
    const inv = Math.round((m.investment_suggested || 0) * factor)
    return { ...m, registrations: reg, schedules: sch, visits: vis, enrollments_new: newEnr, enrollments_returning: retEnr, enrollments: totalEnr, investment_suggested: inv, leads_target: reg, cpa_target: totalEnr > 0 ? Math.round(inv / totalEnr) : 0 }
  })
  scaled.summary.total_new_students_target = Math.round(plan.summary.total_new_students_target * factor)
  scaled.summary.reenrollment_target = Math.round(plan.summary.reenrollment_target * factor)
  scaled.total_investment_suggested = scaled.monthly_targets.reduce((s, m) => s + m.investment_suggested, 0)
  scaled.total_leads_needed = scaled.monthly_targets.reduce((s, m) => s + m.registrations, 0)
  const totalEnr = scaled.monthly_targets.reduce((s, m) => s + m.enrollments, 0)
  scaled.average_cpa = totalEnr > 0 ? Math.round(scaled.total_investment_suggested / totalEnr) : 0
  scaled.summary.realism_score = factor < 0.85 ? 'conservative' : factor > 1.15 ? 'aggressive' : 'realistic'
  return scaled
}
...
export default function CampaignGeneratorModal({
  ...
  const [schoolData, setSchoolData] = useState<SchoolData>({ name: institutionName, city: '', state: '', grades: [], avg_monthly_fee: 0, current_students: 0, start_date: `${new Date().getFullYear()}-08-01`, end_date: `${new Date().getFullYear() + 1}-02-28` })
  ...
  const executionYear = _startDate.getFullYear()
  ...
  useEffect(() => {
    ...
    ;(async () => {
      const { data } = await supabase.from('institutions').select('city, state').eq('id', institutionId).single()
      if (!data) return
  ...
  useEffect(() => {
    if (existingCycle) {
      ...
      if (existingCycle.market_data && Object.keys(existingCycle.market_data).length) setMarketData(existingCycle.market_data)
      if (existingCycle.historical_input?.length) { setHistoricalData(existingCycle.historical_input); setHistoryOption('partial') }
    }
  }, [existingCycle])
  ...
  useEffect(() => {
    if (!isOpen) return
    if (preloadedHistoricalData?.length) { setHistoricalData(preloadedHistoricalData); setHistoryOption('upload') }
    if (preloadedErpFiles?.length) {
      setErpFiles(preloadedErpFiles)
      const feeEntry = preloadedErpFiles.find(f => !f.error && (f.fee ?? 0) > 0)
      const mostRecent = preloadedErpFiles.filter(f => !f.error && f.year > 2000).sort((a, b) => b.year - a.year)[0]
      if (feeEntry?.fee || mostRecent?.total) setSchoolData(s => ({ ...s, ...(feeEntry?.fee ? { avg_monthly_fee: feeEntry.fee! } : {}), ...(mostRecent?.total ? { current_students: mostRecent.total } : {}) }))
    }
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        const { data } = await supabase.from('campaign_cycles').select('*').eq('institution_id', institutionId).eq('status', 'draft').maybeSingle()
        if (!data) {
          const raw = localStorage.getItem(DRAFT_KEY)
          if (!raw) return
          try { const draft = JSON.parse(raw); if (draft.schoolData) setSchoolData(s => ({ ...s, ...draft.schoolData })); if (draft.growthTarget) setGrowthTarget(draft.growthTarget); if (draft.historicalData?.length) setHistoricalData(draft.historicalData); if (draft.historyOption) setHistoryOption(draft.historyOption); if (draft.marketData && Object.keys(draft.marketData).length) setMarketData(draft.marketData); if (draft.generatedPlan) { setGeneratedPlan(draft.generatedPlan); setAdjustedPlan(JSON.parse(JSON.stringify(draft.generatedPlan))); setGenerationMode(draft.generationMode || 'benchmark') }; if (draft.step && draft.step > 1) setStep(draft.step) } catch {}
          return
        }
        ...
        if ((data.erp_files as unknown[])?.length) setErpFiles(data.erp_files as ErpFileEntry[])
      ...
      try {
        ...
        if (setupCycle) {
          ...
          if (sd) setSchoolData(prev => ({ ...prev, city: sd.city ?? prev.city, state: sd.state ?? prev.state, grades: sd.grades?.length ? sd.grades : prev.grades, avg_monthly_fee: sd.avg_monthly_fee ?? prev.avg_monthly_fee, current_students: sd.current_students ?? prev.current_students }))
          const hd = setupCycle.historical_data as HistoricalYear[] | null
          if (hd?.length) { setHistoricalData(hd); setHistoryOption('upload') }
          const ef = setupCycle.erp_files as ErpFileEntry[] | null
  ...
  const fetchMarket = useCallback(async () => {
    ...
    try {
      const res = await fetch('/api/ai', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ action: 'fetch_ibge', payload: { city: schoolData.city, state: schoolData.state } }) })
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    for (let fi = 0; fi < files.length; fi++) {
      ...
      const isPdf = ext === 'pdf'
      try {
        const content = await new Promise<string>((resolve, reject) => { const reader = new FileReader(); reader.onload = ev => resolve((ev.target?.result as string) || ''); reader.onerror = reject; if (isPdf) reader.readAsDataURL(file); else reader.readAsText(file, 'utf-8') })
    ...
    setErpFiles(prev => { const combined = [...prev, ...newErpFiles]; return combined.filter((f, i) => combined.findIndex(x => x.name === f.name) === i) })
    if (newHistoricalData.length) { setHistoricalData(prev => { const combined = [...prev, ...newHistoricalData]; return combined.reduce<HistoricalYear[]>((acc, row) => { if (!acc.find(r => r.year === row.year)) acc.push(row); return acc }, []).sort((a, b) => a.year - b.year) }); setHistoryOption('upload') }
  ...
  const updateMonthlyCell = (idx: number, field: keyof MonthlyTarget, value: number) => {
    ...
    updated.monthly_targets = updated.monthly_targets.map((m, i) => {
      ...
      if (field === 'enrollments_new' || field === 'enrollments_returning') newM.enrollments = (newM.enrollments_new ?? 0) + (newM.enrollments_returning ?? 0)
      if (['investment_suggested','enrollments_new','enrollments_returning','enrollments'].includes(field)) newM.cpa_target = newM.enrollments > 0 ? Math.round(newM.investment_suggested / newM.enrollments) : 0
      return newM
    })
    ...
    const totalEnr = updated.monthly_targets.reduce((s, m) => s + (m.enrollments || 0), 0)
    updated.average_cpa = totalEnr > 0 ? Math.round(updated.total_investment_suggested / totalEnr) : 0
  ...
  const applyCampaign = async () => {
    ...
    try {
      ...
      const cycleData: CycleData = { institution_id: institutionId, year: executionYear, label: `Campanha ${executionYear}`, start_date: sd, end_date: ed, target_new_students: adjustedPlan.summary.total_new_students_target, target_reenrollment_rate: adjustedPlan.summary.reenrollment_rate_target, base_students: schoolData.current_students, projected_cpa: adjustedPlan.average_cpa, monthly_targets: adjustedPlan.monthly_targets, market_data: marketData, historical_input: historicalData, generation_mode: generationMode, ai_reasoning: adjustedPlan.summary.reasoning, realism_score: adjustedPlan.summary.realism_score, applied_at: new Date().toISOString() }

      if (isAdjustMode) {
        const { data: activeCycle } = await supabase.from('campaign_cycles').select('id').eq('institution_id', institutionId).eq('status', 'active').maybeSingle()
        await supabase.from('campaign_change_requests').insert({ institution_id: institutionId, cycle_id: activeCycle?.id ?? null, requested_by: currentUserId ?? null, requested_by_name: currentUserName ?? null, changes: cycleData, status: 'pending', created_at: new Date().toISOString() })
        if (isMounted.current) { setDraftToast('📋 Solicitação de ajuste enviada. Aguardando aprovação do administrador.'); setTimeout(() => { if (isMounted.current) setDraftToast(null); onClose() }, 2500) } else onClose()
        return
      }
    ...
    <div style={{ position: 'fixed', inset: 0, zIndex: 1000, background: 'rgba(15,23,42,0.7)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 }}>
      <div style={{ background: '#fff', borderRadius: 20, width: '100%', maxWidth: step === 5 ? 1100 : 680, maxHeight: '95vh', display: 'flex', flexDirection: 'column', overflow: 'hidden', boxShadow: '0 32px 80px rgba(0,0,0,0.25)' }}>
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '0 28px' }}>
          ...
          {step === 4 && <StepFour loading={loadingGenerate} msgIdx={genMsgIdx} msgs={GEN_MSGS} progress={genProgress} error={error} onRetry={() => generateCampaign()} />}
          {step === 5 && adjustedPlan && (
            <StepFive
              plan={adjustedPlan}
        ...
        <div style={{ padding: '16px 28px 20px', borderTop: '1px solid #E2E8F0', flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: '#FAFAFA' }}>
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            {step === 5 && (
              <button onClick={saveDraft} style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '9px 16px', borderRadius: 10, border: `1px solid ${draftSaved ? '#bbf7d0' : '#E2E8F0'}`, background: draftSaved ? '#f0fdf4' : '#fff', fontSize: 13, color: draftSaved ? '#16a34a' : '#64748B', cursor: 'pointer' }}>
                {draftSaved && <Check size={13} />} {draftSaved ? 'Rascunho salvo!' : 'Salvar rascunho'}
              </button>
            )}
            {step < 3 && (
              <button onClick={async () => {
                if (step === 1) {
                  ...
                  try { await supabase.from('campaign_cycles').upsert({ institution_id: institutionId, year: executionYear, label: `Campanha ${executionYear}`, start_date: schoolData.start_date || `${executionYear}-08-01`, end_date: schoolData.end_date || `${executionYear + 1}-02-28`, status: 'draft', wizard_step: 1, campaign_start_month: campaignStartMonthNum, school_data: { ...schoolData, exits: schoolData.exits ?? {}, total_exits: totalExits }, base_students: schoolData.current_students || 0, target_new_students: 0, target_reenrollment_rate: 85 }, { onConflict: 'institution_id,year' }) } catch {}
                  setStep(2)
                } else if (step === 2) {
                  if (!schoolData.city || !schoolData.state) { setError('Preencha cidade e estado antes de continuar.'); setStep(1); return }
                  try { await supabase.from('campaign_cycles').upsert({ institution_id: institutionId, year: executionYear, label: `Campanha ${executionYear}`, start_date: schoolData.start_date || `${executionYear}-08-01`, end_date: schoolData.end_date || `${executionYear + 1}-02-28`, status: 'draft', wizard_step: 2, campaign_start_month: campaignStartMonthNum, school_data: { ...schoolData, exits: schoolData.exits ?? {}, total_exits: totalExits }, base_students: schoolData.current_students || 0, target_new_students: 0, target_reenrollment_rate: 85, historical_data: historicalData, erp_files: erpFiles }, { onConflict: 'institution_id,year' }) } catch {}
                  setStep(3); fetchMarket()
                }
            ...
            {step === 3 && !loadingMarket && Object.keys(marketData).length > 0 && (
              <button onClick={async () => {
                if (!schoolData.city || !schoolData.state) { setError('Preencha cidade e estado no Passo 1.'); setStep(1); return }
                try { await supabase.from('campaign_cycles').upsert({ institution_id: institutionId, year: executionYear, label: `Campanha ${executionYear}`, start_date: schoolData.start_date || `${executionYear}-08-01`, end_date: schoolData.end_date || `${executionYear + 1}-02-28`, status: 'draft', wizard_step: 3, campaign_start_month: campaignStartMonthNum, school_data: { ...schoolData, exits: schoolData.exits ?? {}, total_exits: totalExits }, base_students: schoolData.current_students || 0, target_new_students: 0, target_reenrollment_rate: 85, historical_data: historicalData, erp_files: erpFiles, market_data: marketData }, { onConflict: 'institution_id,year' }) } catch {}
            ...
            {step === 5 && (
              <button onClick={() => setShowConfirm(true)} disabled={applying} style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '9px 20px', borderRadius: 10, background: '#00A896', color: '#fff', border: 'none', fontSize: 13, fontWeight: 600, cursor: 'pointer', opacity: applying ? 0.7 : 1 }}>
                {applying ? <Loader2 size={14} className="animate-spin" /> : <Check size={14} />} Aplicar metas ao sistema
              </button>
            )}
          </div>
        </div>
        ...
        <div style={{ position: 'fixed', inset: 0, zIndex: 1100, background: 'rgba(15,23,42,0.7)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 }}>
          <div style={{ background: '#fff', borderRadius: 16, padding: 28, maxWidth: 440, width: '100%', boxShadow: '0 20px 60px rgba(0,0,0,0.2)' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 16 }}>
              <div style={{ width: 36, height: 36, borderRadius: 10, background: '#D1FAE5', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><Shield size={16} color="#00A896" /></div>
            ...
            <div style={{ display: 'flex', gap: 10, justifyContent: 'flex-end' }}>
              ...
              <button onClick={applyCampaign} disabled={applying} style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '9px 18px', borderRadius: 9, background: '#00A896', color: '#fff', border: 'none', fontSize: 13, fontWeight: 600, cursor: 'pointer' }}>
                {applying ? <Loader2 size={13} className="animate-spin" /> : <Check size={13} />} Confirmar e aplicar
              </button>
            </div>
          </div>
        </div>
...
// ─── Passo 1 ─────────────────────────────────────────────────────
function StepOne({ schoolData, setSchoolData, growthTarget, setGrowthTarget, institutionPrefill, totalExits }: { schoolData: SchoolData; setSchoolData: React.Dispatch<React.SetStateAction<SchoolData>>; growthTarget: GrowthTarget; setGrowthTarget: React.Dispatch<React.SetStateAction<GrowthTarget>>; institutionPrefill: { city: string; state: string } | null; totalExits: number }) {
  const thisYear = new Date().getFullYear()
  const PrefillBadge = () => <span style={{ marginLeft: 6, fontSize: 10, fontWeight: 500, color: '#0369a1', background: '#e0f2fe', borderRadius: 99, padding: '2px 7px' }}>Preenchido automaticamente do cadastro</span>
  return (
      ...
      <div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 10, marginTop: 8 }}>
          {(['percentage', 'absolute', 'students'] as const).map(type => {
            ...
            return (
              <div key={type} onClick={() => setGrowthTarget(t => ({ ...t, type }))} style={{ padding: 14, borderRadius: 12, cursor: 'pointer', border: active ? '2px solid #00A896' : '1.5px solid #E2E8F0', background: active ? '#E6F7F5' : '#F8FAFC' }}>
                <div style={{ fontSize: 13, fontWeight: 600, color: active ? '#00A896' : '#475569', marginBottom: 8 }}>{labels[type]}</div>
                <input type="number" style={{ width: '100%', padding: '6px 10px', borderRadius: 7, border: '1px solid #E2E8F0', fontSize: 14, background: '#fff', outline: 'none' }} placeholder={placeholders[type]} value={growthTarget.type === type ? growthTarget.value || '' : ''} onClick={e => e.stopPropagation()} onChange={e => setGrowthTarget({ type, value: parseFloat(e.target.value) || 0 })} />
              </div>
            )
          })}
        </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            {schoolData.grades.map(grade => (
              <div key={grade} style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                ...
                <input type="number" min={0} placeholder="0" value={schoolData.exits?.[grade] ?? ''} onChange={e => setSchoolData(prev => ({ ...prev, exits: { ...prev.exits, [grade]: parseInt(e.target.value) || 0 } }))} style={{ width: 80, height: 34, padding: '0 8px', borderRadius: 7, border: '1px solid #E2E8F0', fontSize: 13, background: '#F8FAFC', outline: 'none' }} />
                <span style={{ fontSize: 12, color: '#94A3B8' }}>alunos formandos</span>
              </div>
            ))}
          </div>
          {totalExits > 0 && <p style={{ fontSize: 12, color: '#64748B', marginTop: 10 }}>Total de saídas naturais: <strong>{totalExits} alunos</strong></p>}
        </div>
      )}
    </div>
  )
...
// ─── Passo 2 ─────────────────────────────────────────────────────
function StepTwo({ historyOption, setHistoryOption, historicalData, setHistoricalData, loadingFile, multiFileProgress, erpFiles, setErpFiles, fileInputRef, onFileChange }: { historyOption: 'upload' | 'partial' | 'none' | null; setHistoryOption: (v: 'upload' | 'partial' | 'none') => void; historicalData: HistoricalYear[]; setHistoricalData: React.Dispatch<React.SetStateAction<HistoricalYear[]>>; loadingFile: boolean; multiFileProgress: string | null; erpFiles: ErpFileEntry[]; setErpFiles: React.Dispatch<React.SetStateAction<ErpFileEntry[]>>; fileInputRef: React.RefObject<HTMLInputElement>; onFileChange: (e: React.ChangeEvent<HTMLInputElement>) => void }) {
  const addYear = () => setHistoricalData(d => [...d, { year: new Date().getFullYear() - 1 - d.length, total_students: 0, new_enrollments: 0, reenrollments: 0, transfers: 0 }])
  return (
    <div style={{ paddingBottom: 24 }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 10, marginBottom: 20 }}>
        {([{ key: 'upload', icon: '📂', label: 'Sim, tenho dados — vou subir agora', sub: 'Aceita CSV, Excel ou PDF do seu sistema' }, { key: 'partial', icon: '📊', label: 'Tenho dados parciais (1 ano apenas)', sub: 'Preencha manualmente abaixo' }, { key: 'none', icon: '🆕', label: 'Não tenho / é o primeiro ano no sistema', sub: 'A IA usará benchmarks do setor' }] as const).map(opt => {
          ...
          return <div key={opt.key} onClick={() => setHistoryOption(opt.key)} style={{ padding: '14px 18px', borderRadius: 12, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 14, border: active ? '2px solid #00A896' : '1.5px solid #E2E8F0', background: active ? '#E6F7F5' : '#F8FAFC' }}>
            ...
            <div><div style={{ fontSize: 14, fontWeight: 600, color: active ? '#00A896' : '#1A2B4A' }}>{opt.label}</div><div style={{ fontSize: 12, color: '#94A3B8', marginTop: 2 }}>{opt.sub}</div></div>
            ...
            <div style={{ marginBottom: 16 }}>
              {erpFiles.length > 0 && (
                <div style={{ display: 'flex', flexDirection: 'column', gap: 8, marginBottom: 12 }}>
                  {erpFiles.map((f, i) => (
                    <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 14px', borderRadius: 10, background: f.error ? '#fef2f2' : '#f0fdf4', border: `1px solid ${f.error ? '#fca5a5' : '#bbf7d0'}` }}>
...
// ─── Passo 3 ─────────────────────────────────────────────────────
function StepThree({ loading, msgIdx, msgs, marketData, city, error, onRetry }: { loading: boolean; msgIdx: number; msgs: string[]; marketData: MarketData; city: string; error: string | null; onRetry: () => void }) {
  ...
  return (
    <div style={{ paddingBottom: 24 }}>
      ...
      {loading && <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '40px 20px', gap: 16 }}><div style={{ width: 56, height: 56, borderRadius: '50%', background: '#E6F7F5', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><Loader2 size={26} color="#00A896" className="animate-spin" /></div><p style={{ fontSize: 14, color: '#475569', textAlign: 'center' }}>{msgs[msgIdx]}</p></div>}
      {error && !loading && <div style={{ padding: 20, background: '#FFF1F2', borderRadius: 12, border: '1px solid #FECDD3', marginBottom: 16 }}><p style={{ fontSize: 13, color: '#BE123C', marginBottom: 12 }}>{error}</p><button onClick={onRetry} style={{ fontSize: 13, color: '#00A896', background: '#E6F7F5', border: 'none', borderRadius: 8, padding: '7px 14px', cursor: 'pointer' }}>Tentar novamente</button></div>}
      {hasData && !loading && (
        <div>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 16 }}>
            <StatCard color="#3B82F6" bg="#EFF6FF" icon={<Users size={16} color="#3B82F6" />} label={`Crianças em idade escolar em ${city}`} value={marketData.school_age_population?.toLocaleString('pt-BR') || 'N/D'} />
            <StatCard color="#10B981" bg="#ECFDF5" icon={<BarChart3 size={16} color="#10B981" />} label="Estudam em escola particular" value={marketData.private_school_rate ? `${(marketData.private_school_rate * 100).toFixed(1)}%` : 'N/D'} />
            <StatCard color="#00A896" bg="#E6F7F5" icon={<TrendingUp size={16} color="#00A896" />} label="Crescimento do setor ao ano" value={marketData.sector_growth_rate ? `${(marketData.sector_growth_rate * 100).toFixed(1)}%` : 'N/D'} />
            <StatCard color="#6B7280" bg="#F9FAFB" icon={<MapPin size={16} color="#6B7280" />} label="Média de alunos por escola particular" value={marketData.average_students_per_school?.toLocaleString('pt-BR') || 'N/D'} />
          </div>
          {marketData.confidence && <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 12 }}><span style={{ padding: '4px 10px', borderRadius: 999, fontSize: 11, fontWeight: 600, background: marketData.confidence === 'high' ? '#D1FAE5' : marketData.confidence === 'medium' ? '#FEF3C7' : '#F1F5F9', color: marketData.confidence === 'high' ? '#065F46' : marketData.confidence === 'medium' ? '#92400E' : '#475569' }}>{marketData.confidence === 'high' ? '✓ Alta confiança' : marketData.confidence === 'medium' ? '~ Confiança média' : '? Dados estimados'}</span></div>}
          {marketData.notes && <p style={{ fontSize: 12, color: '#64748B', background: '#F8FAFC', padding: '10px 14px', borderRadius: 9, lineHeight: 1.6 }}>💡 {marketData.notes}</p>}
          <p style={{ fontSize: 11, color: '#94A3B8', marginTop: 10 }}>Fonte: {marketData.data_source || 'IBGE Censo 2022 / Censo Escolar MEC 2023'}</p>
        </div>
      )}
    </div>
  )
...
// ─── Passo 4 ─────────────────────────────────────────────────────
function StepFour({ loading, msgIdx, msgs, progress, error, onRetry }: { loading: boolean; msgIdx: number; msgs: string[]; progress: number; error: string | null; onRetry: () => void }) {
  return (
          ...
          <div style={{ background: '#F8FAFC', borderRadius: 12, padding: 14, border: '1px solid #E2E8F0' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 6 }}>
              {[{ level: 0, label: 'Conservador', icon: '🛡️', desc: '−25%' }, { level: 1, label: 'Realista', icon: '🎯', desc: 'IA sugeriu' }, { level: 2, label: 'Agressivo', icon: '🚀', desc: '+30%' }].map(opt => (
                <button key={opt.level} onClick={() => onAmbitiousChange(opt.level)} style={{ padding: '8px 4px', borderRadius: 9, cursor: 'pointer', fontSize: 11, fontWeight: 600, border: ambitiousLevel === opt.level ? '2px solid #00A896' : '1.5px solid #E2E8F0', background: ambitiousLevel === opt.level ? '#E6F7F5' : '#fff', color: ambitiousLevel === opt.level ? '#00A896' : '#64748B', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 2 }}>
          ...
          {plan.pre_campaign && monthsUntilCampaign > 0 && (
            <div style={{ background: '#FFFBEB', border: '1px solid #FDE68A', borderRadius: 12, padding: '14px 16px' }}>
              ...
              <ul style={{ margin: 0, padding: 0, listStyle: 'none', display: 'flex', flexDirection: 'column', gap: 4 }}>
                {plan.pre_campaign.key_actions.map((action, i) => <li key={i} style={{ display: 'flex', gap: 8, alignItems: 'flex-start', fontSize: 12, color: '#78350F' }}><span style={{ color: '#D97706', fontWeight: 700, flexShrink: 0 }}>✓</span>{action}</li>)}
              </ul>
            </div>
          )}
                ...
                <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12 }}>
                  <thead>
                    <tr style={{ background: '#F8FAFC' }}>
                      {['Mês', 'Cadastros', 'Agendas', 'Visitas', 'Novatos', 'Remat.', 'Total', 'Invest. R$', 'CPA R$'].map(h => <th key={h} style={{ padding: '7px 8px', textAlign: h === 'Mês' ? 'left' : 'center', fontSize: 10, fontWeight: 700, color: '#64748B', borderBottom: '1px solid #E2E8F0', whiteSpace: 'nowrap' }}>{h}</th>)}
                    </tr>
                  </thead>
                  <tbody>
                    ...
                    <tr style={{ background: '#F0FDFB', borderTop: '2px solid #D1FAE5' }}>
                      <td style={{ padding: '7px 8px', fontWeight: 700, color: '#1A2B4A', fontSize: 11 }}>TOTAL</td>
                      <td style={{ padding: '7px 8px', textAlign: 'center', fontWeight: 700, fontSize: 11 }}>{totals.registrations}</td>
                      <td style={{ padding: '7px 8px', textAlign: 'center', fontWeight: 700, fontSize: 11 }}>{totals.schedules}</td>
                      <td style={{ padding: '7px 8px', textAlign: 'center', fontWeight: 700, fontSize: 11 }}>{totals.visits}</td>
                      <td style={{ padding: '7px 8px', textAlign: 'center', fontWeight: 700, color: '#1d4ed8', fontSize: 11 }}>{totals.enrollments_new}</td>
                      <td style={{ padding: '7px 8px', textAlign: 'center', fontWeight: 700, color: '#16a34a', fontSize: 11 }}>{totals.enrollments_returning}</td>
                      <td style={{ padding: '7px 8px', textAlign: 'center', fontWeight: 700, color: '#0d9488', fontSize: 11 }}>{totals.enrollments}</td>
                      <td style={{ padding: '7px 8px', textAlign: 'center', fontWeight: 700, fontSize: 11 }}>R$ {totals.investment_suggested.toLocaleString('pt-BR')}</td>
                      <td style={{ padding: '7px 8px', textAlign: 'center', fontWeight: 700, fontSize: 11 }}>{totals.enrollments > 0 ? `R$ ${Math.round(totals.investment_suggested / totals.enrollments).toLocaleString('pt-BR')}` : '—'}</td>
                    </tr>
                  </tbody>
                </table>
              </div>
                  ...
                  <tbody>
                    ...
                    <tr style={{ background: '#FFFBEB', borderTop: '2px solid #FCD34D' }}>
                      ...
                      <td style={{ padding: '9px 12px', textAlign: 'center', fontWeight: 800, color: totals.enrollments_returning >= reenrollTarget ? '#16a34a' : '#f59e0b' }}>
                        {reenrollTarget > 0 ? ((totals.enrollments_returning / reenrollTarget) * 100).toFixed(1) : '0.0'}%
                      </td>
                      <td style={{ padding: '9px 12px', textAlign: 'center', fontWeight: 700, color: '#94A3B8' }}>{reenrollTarget}</td>
                      <td style={{ padding: '9px 12px', textAlign: 'center', fontWeight: 800, color: '#00A896' }}>{totals.enrollments_returning}</td>
                      <td style={{ padding: '9px 12px', textAlign: 'center', fontWeight: 700, color: totals.enrollments_returning >= reenrollTarget ? '#16a34a' : '#dc2626' }}>
                        {totals.enrollments_returning >= reenrollTarget ? '+' : ''}{totals.enrollments_returning - reenrollTarget}
                      </td>
                    </tr>
                  </tbody>
                </table>
...
function StatCard({ color, bg, icon, label, value }: { color: string; bg: string; icon: React.ReactNode; label: string; value: string }) {
  return <div style={{ background: bg, borderRadius: 12, padding: '12px 14px' }}><div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6 }}>{icon}<span style={{ fontSize: 11, color, fontWeight: 600 }}>{label}</span></div><div style={{ fontSize: 20, fontWeight: 800, color }}>{value}</div></div>
}

function KpiCard({ label, value, unit, color, raw }: { label: string; value: number | string; unit: string; color: string; raw?: boolean }) {
  return <div style={{ background: '#F8FAFC', borderRadius: 10, padding: '10px 12px', border: '1px solid #E2E8F0' }}><div style={{ fontSize: 10, color: '#94A3B8', marginBottom: 4 }}>{label}</div><div style={{ fontSize: 17, fontWeight: 800, color }}>{raw ? value : `${value}${unit}`}</div></div>
}