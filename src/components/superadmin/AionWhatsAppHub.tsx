...
// ─── MessageBubble ────────────────────────────────────────────────────────────
function MessageBubble({ msg, onImageClick, contactName }: { msg: Message; onImageClick?: (url: string) => void; contactName?: string }) {
...
const AION_CLOSING_IDS = new Set<string>()

export default function AionWhatsAppHub() {
  const { user } = useAuth()
  const effectiveInstitutionId = ''
  const isAionInbox = true
  ...
  // closingIdsRef replaced by module-level AION_CLOSING_IDS
  ...
  const handleLinkLead = async (leadId: string) => {
    ...
    const { data: lead } = await supabase
      .from('crm_leads')
      .select('id, name, phone, email, stage, origin, created_at')
      .eq('id', leadId)
      .single()
    if (lead) {
      setLeadData(lead)
  ...
  const searchLeads = async (q: string) => {
    ...
    const { data } = await supabase
      .from('crm_leads')
      .select('id, name, phone, email, stage, origin')
      .or(`name.ilike.%${q}%,phone.ilike.%${q}%`)
      .limit(8)
    setLeadResults((data || []).map((l: any) => ({
      id: l.id,
  ...
  const addMessageToConversations = (newMsg: WhatsappMessage) => {
    ...
    setConversations(prev => {
      ...
      if (existing) {
        ...
        // era anexada ao lado do placeholder, que nunca era removido.
  ...
  const loadMessages = async () => {
    ...
    // pg_stat_statements na mesma query em WhatsAppHub.tsx: 87.817 chamadas,
    // 400ms média, 9h46min de tempo total acumulado — ver correção lá e em
    // AionInboxHub.tsx). Aqui essa função ainda roda a cada 60s via
    // setInterval (mais abaixo), então o ganho é maior ainda. whatsapp_
    // conversations já tem last_message/last_message_at/unread_count/etc pra
    // montar a lista; buildConversations usa esses campos quando não há
    // mensagens carregadas. O thread de cada conversa é buscado sob demanda
    // só quando ela é aberta (useEffect de lazy-load logo abaixo).
    const msgs: any[] = []
    let convs: any[]
    if (isAionInbox) {
  ...
  // limite de 100. Usada pelo lazy-load logo abaixo e por
  // reloadConversationMessages, em vez de cada ponto que precisa atualizar a
  // conversa aberta chamar loadMessages().
  const fetchConversationMessages = async (jid: string): Promise<Message[]> => {
    ...
    if (isAionInbox) {
      const { data } = await supabase
        ...
        .select('id, remote_jid, from_me, message_id, message_type, content, media_url, contact_name, lead_id, timestamp, status, quoted_message_id, quoted_content, quoted_from_me')
        .eq('is_aion_inbox', true)
  ...
  // transferência, pra garantir que o estado local reflita o banco sem
  // esperar o Realtime (que também cobre isso, mas pode chegar com atraso).
  ...
  useEffect(() => {
    ...
    const init = async () => {
      ...
      ;(async () => { try { const { data } = await supabase.from('users').select('id, full_name, email, user_type, role').or('user_type.eq.consultant,role.eq.admin_geral'); if (data) setUsers(data as unknown as UserType[]) } catch {} })()
    ...
    const msgChannel = supabase
      ...
      .on('postgres_changes', {
        ...
        console.log('[REALTIME MSG] INSERT', msgJid, '| AION_CLOSING_IDS tem?', AION_CLOSING_IDS.has(msgJid), AION_CLOSING_IDS.has(normalizeJid(msgJid)))
        if (AION_CLOSING_IDS.has(msgJid) || AION_CLOSING_IDS.has(normalizeJid(msgJid))) {
          console.log('[REALTIME MSG] ignorado — está em AION_CLOSING_IDS')
          return
        }
  ...
  // (loadMessages() parou de trazer mensagens junto — ver comentário lá).
  useEffect(() => {
    if (!activeId) return
  ...
  // Load lead data from crm_leads by phone when active conversation changes
  useEffect(() => {
    if (!activeConv) { setLeadData(null); setEditingLead(false); return }
    const rawPhone = activeConv.phone.replace(/\D/g, '')
    if (!rawPhone) { setLeadData(null); setEditingLead(false); return }
    supabase
      .from('crm_leads')
      .select('id, name, phone, email, empresa, stage, origin, created_at')
  ...
  const filteredConvs = conversations.filter(c => {
    ...
    if (!search || c.name.toLowerCase().includes(search.toLowerCase()) || c.phone.includes(search)) {
      ...
      if (queueFilter === 'general' && c.queue !== 'general') return false
      // read filter
      if (readFilter === 'read' && (c.unreadCount || 0) > 0) return false
      if (readFilter === 'unread') {
        if ((c.unreadCount || 0) === 0) return false
        if (!(['gestor', 'admin', 'superadmin'] as string[]).includes(user?.role ?? '')) {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      if (conv && (conv.status === 'waiting' || !conv.assigned_user_id) && user?.id) {
        const rJid = rawJid(activeId)
        await supabase.from('whatsapp_conversations')
          .update({
            status: 'open',
  ...
  const handleReactivate = async () => {
    if (!activeId || sendingReactivate) return
  ...
  const handleStatusChange = async (status: ConvStatus) => {
    ...
    await supabase.from('whatsapp_conversations')
      ...
      .eq('is_aion_inbox', true)
      .eq('remote_jid', rJid)
    supabase.from('whatsapp_conversation_events').insert({
      institution_id: null,
      remote_jid: rJid,
  ...
  const handleTransfer = async () => {
    if (!activeId || !transferTarget) return
  ...
  const handleContactType = async (type: string) => {
    ...
    const rJid = rawJid(activeId)
    await supabase.from('whatsapp_conversations')
      .update({ contact_type: type, updated_at: new Date().toISOString() })
      .eq('is_aion_inbox', true)
      .eq('remote_jid', rJid)
  ...
  const handleCloseConversation = async () => {
    ...
    AION_CLOSING_IDS.add(rJid)
    AION_CLOSING_IDS.add(convId)
    ...
    const { count } = await supabase.from('whatsapp_conversations')
      .update({ status: 'closed', bot_active: false, assigned_user_id: null, assigned_user_name: null, updated_at: new Date().toISOString() })
      .eq('is_aion_inbox', true)
      .eq('remote_jid', rJid)
    if ((count ?? 0) === 0) {
      console.warn('[CLOSE] 0 linhas atualizadas — verificar formato do JID:', rJid)
    }
    supabase.from('whatsapp_conversation_events').insert({
      institution_id: null,
      remote_jid: rJid,
    ...
    setTimeout(() => {
      console.log('[CLOSE 7] limpando AION_CLOSING_IDS para', rJid)
      AION_CLOSING_IDS.delete(rJid)
      AION_CLOSING_IDS.delete(convId)
    }, 10000)
  ...
  const handleAssignFromClosed = async () => {
    ...
    const rJid = rawJid(activeId)
    await supabase.from('whatsapp_conversations')
      .update({ assigned_user_id: targetUser.id, assigned_user_name: targetUser.full_name, status: 'open', updated_at: new Date().toISOString() })
      .eq('is_aion_inbox', true)
      .eq('remote_jid', rJid)
    await supabase.from('whatsapp_conversation_events').insert({
      institution_id: null,
  ...
  const handleLeaveConversation = async () => {
    ...
    await supabase.from('whatsapp_conversations')
      .update({ status: 'waiting', assigned_user_id: null, assigned_user_name: null, updated_at: new Date().toISOString() })
      .eq('is_aion_inbox', true)
      .eq('remote_jid', rJid)
    supabase.from('whatsapp_conversation_events').insert({
      institution_id: null,
  ...
  const handleCreateLead = async () => {
    if (!leadForm.responsible_name) return
    try {
      const { data: lead, error } = await supabase.from('crm_leads').insert({
        name: leadForm.responsible_name,
        phone: leadForm.phone || activeConv?.phone || '',
      ...
      <div style={{ display: 'flex', flexDirection: 'column', overflow: 'hidden', background: '#F0FDFB', height: '100%' }}>
        ...
        <div style={{ width: isMobile ? '100%' : 320, flexShrink: 0, display: isMobile && mobilePanel === 'chat' ? 'none' : 'flex', flexDirection: 'column', background: '#FFFFFF', borderRight: '1px solid #D1FAE5', overflow: 'hidden' }}>
          ...
          <div style={{ borderBottom: '1px solid #D1FAE5' }}>
            ...
            {/* Row 2: Queue filter pills */}
            <div style={{ display: 'flex', padding: '0 12px 6px', gap: 6, alignItems: 'center', flexWrap: 'wrap' }}>
              {([
                { key: 'all',     label: 'Todas'   },
          ...
          <div className="wa-scrollbar" style={{ flex: 1, overflowY: 'auto', padding: '16px', display: 'flex', flexDirection: 'column', gap: 2, backgroundImage: 'radial-gradient(circle at 1px 1px, #ccf0ec 1px, transparent 0)', backgroundSize: '24px 24px', backgroundColor: '#f7fefe' }}>
            ...
            {msgGroups.map((group, gi) => (
              <div key={gi}>
                ...
                {group.msgs.map(msg => (
                  <MessageBubble key={msg.id} msg={msg} onImageClick={url => setLightboxUrl(url)} contactName={activeConv?.name || 'Contato'} />
                ))}
              </div>
            ))}
                            ...
                            <div style={{ display: 'flex', gap: 6 }}>
                              <button onClick={async () => {
                                ...
                                if (editForm.contact_type && editForm.contact_type !== (activeConv.contact_type || '')) {
                                  await supabase.from('whatsapp_conversations')
                                    .update({ contact_type: editForm.contact_type, updated_at: new Date().toISOString() })
                                    .eq('is_aion_inbox', true)
                                    .eq('remote_jid', rJid)
                            ...
                            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                              ...
                              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '8px 10px', background: '#f0fdfb', borderRadius: 9, border: '1px solid #d1fae5' }}>
                                <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                                  ...
                                  <div>
                                    ...
                                    {leadData.empresa && (
                                      <p style={{ margin: 0, fontSize: 11, color: '#64748B' }}>{leadData.empresa}</p>
                                    )}
                                  </div>
                                </div>
                                ...
                                <div style={{ display: 'flex', flexDirection: 'column', gap: 5, padding: '0 2px' }}>
                                  ...
                                  {(() => {
                                    const STAGE_CFG: Record<string, { label: string; bg: string; color: string }> = {
                                      interesse:    { label: 'Interesse',    bg: '#DBEAFE', color: '#1D4ED8' },
                                      qualificacao: { label: 'Qualificação', bg: '#EDE9FE', color: '#6D28D9' },
                                      proposta:     { label: 'Proposta',     bg: '#FEF3C7', color: '#B45309' },
                                      negociacao:   { label: 'Negociação',   bg: '#FFEDD5', color: '#C2410C' },
                                      fechado:      { label: 'Fechado',      bg: '#D1FAE5', color: '#065F46' },
                                      cliente:      { label: 'Cliente',      bg: '#CCFBF1', color: '#0F766E' },
                                    }
                                    const cfg = STAGE_CFG[leadData.stage] || { label: leadData.stage || '—', bg: '#F1F5F9', color: '#64748B' }
                                    return (
                                      <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                                        <span style={{ fontSize: 11, color: '#64748B' }}>Estágio</span>
                                  ...
                                  {leadData.empresa && (
                                    <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 12 }}>
                                      ...
                                      <span style={{ color: '#1A2B4A', fontWeight: 500 }}>{leadData.empresa}</span>
                                    </div>
                                  )}
                                  {leadData.email && (
                              ...
                              {editingLead && (
                                <div style={{ display: 'flex', flexDirection: 'column', gap: 7, background: '#f0fdfb', borderRadius: 10, padding: '10px 12px', border: '1px solid #d1fae5' }}>
                                  ...
                                  <div>
                                    <label style={{ display: 'block', fontSize: 11, fontWeight: 500, color: '#64748B', marginBottom: 3 }}>Estágio</label>
                              ...
                              {/* Actions row */}
                              <button onClick={() => navigate('/super-admin/crm')}
