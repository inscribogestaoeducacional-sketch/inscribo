CONTEXTO DO PROJETO:
Sistema Áion Edu — React + TypeScript + Supabase + Vercel
Supabase projeto: syxxuumxkhhnoqrxporj
Stack: React, TypeScript, TailwindCSS, Supabase, React Router

TAREFA 1 — Edge Function autentique-webhook
Criar arquivo supabase/functions/autentique-webhook/index.ts:
...
serve(async (req) => {
  ...
  try {
    ...
    // Autentique envia event com document info
    ...
    // Busca contrato pelo autentique_document_id
    ...
    // Verifica se todos assinaram (signed event)
    ...
    if (isSigned) {
      ...
      // Atualiza instituição para pending_payment
      ...
      // Envia email ao gestor com link de pagamento se existir
...
Após criar, rodar:
bashsupabase functions deploy autentique-webhook

TAREFA 2 — InstitutionDetails.tsx COMPLETO
Substituir o arquivo src/components/superadmin/InstitutionDetails.tsx completo:
          ...
          <div className="space-y-4">
            ...
            <div className="bg-white rounded-2xl border border-gray-200 p-5">
              ...
              {!cycle ? (
                ...
                <div className="space-y-3">
                  {[
                    ['Status', { active: 'Ativa', released: 'Liberada', setup: 'Em setup', draft: 'Rascunho', completed: 'Concluída' }[cycle.status] || cycle.status],
...
TAREFA 3 — AdminSchools.tsx — abrir InstitutionDetails ao clicar
No arquivo src/components/superadmin/AdminSchools.tsx, encontra o botão de detalhes:
typescript<button onClick={() => setDetailModal(inst)} className="p-1.5 text-gray-400 hover:text-cyan-600 hover:bg-cyan-50 rounded-lg" title="Detalhes">
  <Eye className="w-3.5 h-3.5" />
</button>
Substitui por:
typescript<button onClick={() => navigate(`/super-admin/schools/${inst.id}`)} className="p-1.5 text-gray-400 hover:text-cyan-600 hover:bg-cyan-50 rounded-lg" title="Detalhes">
  <Eye className="w-3.5 h-3.5" />
</button>
Adiciona o import do useNavigate se não existir:
typescriptimport { useNavigate } from 'react-router-dom'
E adiciona dentro do componente:
typescriptconst navigate = useNavigate()

TAREFA 4 — Rota InstitutionDetails no App.tsx
Encontra onde estão as rotas de super admin (provavelmente algo como /super-admin/schools) e adiciona:
typescript<Route path="/super-admin/schools/:id" element={<InstitutionDetails />} />
Adiciona o import:
typescriptimport InstitutionDetails from './components/superadmin/InstitutionDetails'

TAREFA 5 — ConsultantContracts.tsx — trocar ZapSign por Autentique
No arquivo src/components/superadmin/ConsultantContracts.tsx, encontra:
typescriptconst { error: zapErr } = await supabase.functions.invoke('zapsign', {
  body: {
    contract_id: contract.id,
...
Substitui por:
typescriptconst { error: autErr } = await supabase.functions.invoke('autentique', {
  body: {
...
Também atualiza o label no formulário. Encontra:
typescriptlabel: 'Signatário ZapSign (opcional)',
Substitui por:
typescriptlabel: 'Signatário Autentique (opcional)',
E o botão. Encontra:
typescript<><Send className="w-4 h-4" /> Enviar via ZapSign</>
Substitui por:
typescript<><Send className="w-4 h-4" /> Enviar via Autentique</>

TAREFA 6 — AdminContracts.tsx — remover email duplicado
No arquivo src/components/superadmin/AdminContracts.tsx, dentro de handleSave e handleResendAutentique, remove os blocos que chamam send-email com tipo contract_sign pois a Autentique já envia automaticamente o email para o signatário.
Encontra e remove este bloco dentro de handleSave:
typescript// Envia email com link de assinatura via Brevo
if (autData?.signUrl) {
  await supabase.functions.invoke('send-email', {
    body: {
      type: 'contract_sign',
...
E remove este bloco dentro de handleResendAutentique:
typescriptif (data?.signUrl) {
  await supabase.functions.invoke('send-email', {
    body: {
      type: 'contract_sign',
...
TAREFA 7 — PendingScreen.tsx — tela de bloqueio para gestor
Criar arquivo src/components/PendingScreen.tsx:
typescriptimport { FileText, CreditCard, Clock, MessageCircle } from 'lucide-react'
...
TAREFA 8 — Bloquear gestor no App.tsx
No arquivo src/App.tsx, encontra onde o usuário autenticado é redirecionado (dentro do componente principal ou de um ProtectedRoute).
Adiciona esta lógica para usuários do tipo school_user:
typescript// No componente que renderiza rotas protegidas para school_user
// Após verificar autenticação, verificar status da escola:
...
Adiciona o import:
typescriptimport PendingScreen from './components/PendingScreen'

TAREFA 9 — Deploy da Edge Function
Após criar o arquivo autentique-webhook/index.ts, rodar:
bashsupabase functions deploy autentique-webhook

IMPORTANTE — Observações finais:

Não altere nenhum arquivo não mencionado
Mantenha todos os imports existentes em cada arquivo
Commit com mensagem: feat: institution details completo, autentique-webhook, pending screen, rotas sincronizadas
Para a rota do InstitutionDetails, verifique onde as outras rotas de super-admin estão definidas no App.tsx e adicione no mesmo padrão
O AdminSchools ainda pode manter o modal de detalhes rápidos via SchoolDetailModal para WhatsApp — mas o botão de olho agora navega para InstitutionDetails